! this code is a F2PY subroutine for Python to calculate the centre of density adjusted positions. It takes filename, number of snaps, and number of stars as input and provides the COD-adjusted Radius/Position vector
...
SUBROUTINE COD_Radius(Fname,Nsnaps,Nstars,Codrad)
   ...
   DOUBLE PRECISION :: codrad(1:101,1:3,1:Nstars)
   REAL :: pi

   INTEGER :: dlist(1:101,1:Nstars)   
   ...
   CALL heapsort(nstars,real(sdist(i,1:nstars)),dlist(i,1:nstars))
      ...
      END DO
      cod(i,1) = SUM (dens(i,1:Nstars)*rad(i,1,1:Nstars))/ SUM ( dens(i,1:Nstars) )
      cod(i,2) = SUM (dens(i,1:Nstars)*rad(i,2,1:Nstars))/ SUM ( dens(i,1:Nstars) )
      cod(i,3) = SUM (dens(i,1:Nstars)*rad(i,3,1:Nstars))/ SUM ( dens(i,1:Nstars) )
