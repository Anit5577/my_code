Subroutine Bound_unbound_stars(Fname,Nsnaps,Nstars,Boundenergy)
  ...
  IMPLICIT NONE
  INTEGER :: snap(1:101,1:500)   ! this represents the snapshot number
  INTEGER :: id(1:101,1:500)   ! this represents the id of the star
  INTEGER :: i,j,k, nsnaps,nstars
  INTEGER :: l(1:101)
  CHARACTER*35 :: fname
  DOUBLE PRECISION :: mass(1:101,1:500)  !this represents the mass
  DOUBLE PRECISION :: cmass(1:101,1:500)  !this is the cumulative mass
  DOUBLE PRECISION :: halfmass(1:101,1:500)
  DOUBLE PRECISION :: snapmass(1:101,1:500) !total cluster mass per snapshot
  REAL :: age(1:101,1:500) !this is the time
  DOUBLE PRECISION :: radvec(1:101,1:3,1:500) !this is the position vector
  DOUBLE PRECISION :: nradvec(1:101,1:3,1:500) !this is the radius vector value
  DOUBLE PRECISION :: velvec(1:101,1:3,1:500) !this is the velocity vector
  DOUBLE PRECISION :: velcod(1:101,1:3) !this is the velocity vector
  DOUBLE PRECISION :: veldif(1:101,1:3,1:500) !this is the velocity vector at centre of density
  DOUBLE PRECISION :: poten(1:101,1:500) !this is the potential energy of each star
  DOUBLE PRECISION :: kinen(1:101,1:500) !this is the kineteic energy of each star
  DOUBLE PRECISION :: boundenergy(1:101,1:500) !this is the kineteic energy of each star
  DOUBLE PRECISION :: raddif(1:101,1:500)
  DOUBLE PRECISION :: dens(1:101,1:500) !this is the radius vector value
  DOUBLE PRECISION :: denv(1:101,1:500) !this is the radius vector value
  DOUBLE PRECISION :: top(1:101,1:3,1:500) !this is the radius vector value
  ...
  DOUBLE PRECISION :: dist(1:101,1:3,1:500) !this is the distance vector value
  DOUBLE PRECISION :: vdist(1:101,1:500) !this is the distance vector value 
  DOUBLE PRECISION :: veldist(1:101,1:3,1:500) !this is the distance vector value
  DOUBLE PRECISION :: sdist(1:101,1:500) !this is the square distance value
  REAL :: pi
  REAL :: G !graviational constant
  REAL :: Msun !Solar mass
  REAL :: pc ! radius conversion factor from parsec to metre
     
  INTEGER :: vlist(1:101,1:500)
...
! calculate the centre of density vector for each snap by multiplying each stars density with its radius vector, then summing all of these values up and dividing it by the sum of all densities.     
      ...
      cov(i,1) = SUM (denv(i,1:500)*velvec(i,1,1:500))/ SUM ( denv(i,1:500) )
      cov(i,2) = SUM (denv(i,1:500)*velvec(i,2,1:500))/ SUM ( denv(i,1:500) )
      cov(i,3) = SUM (denv(i,1:500)*velvec(i,3,1:500))/ SUM ( denv(i,1:500) )
